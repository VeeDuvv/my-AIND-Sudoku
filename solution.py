	...
	while not stalled:
		...
		for box, value in values.items():
			...
			if len(value) > 1:
				...
				# for box's peers
				for set_of_peer in all_peers:
					for row_peer in set_of_peer:
						#if its value is equal to box's and it isn't the box itself
						if box != row_peer and values[box] == values[row_peer]:
							# found naked twin
							# print("Found naked twin", box, row_peer)
							for new_row_peer in row_peers:
								if len(values[new_row_peer]) <= len(values[box]):
									#print("length of new row peer value is not LTEQ box value")
									continue
								elif new_row_peer == box or new_row_peer == row_peer:
									...
									count_in_peer = 0
									for s in values[box]:
										if s in values[new_row_peer]:
											#print("\treplacing", s, " in ", values[new_row_peer])
											assign_value(values, new_row_peer, values[new_row_peer].replace(s,""))
